...
namespace PostProcess {
    ...
    void GuassianBlur(const ColorBuffer& colorBuffer){
        ColorBuffer source = colorBuffer;
    ...
    void Edge(const ColorBuffer& colorBuffer, uint8_t threshold){
        ...
        for (int i = 0; i < colorBuffer.width * colorBuffer.height; i++) {
            ...
            for (int iy = -1; iy <= 1; iy++)
            {
                for (int ix = -1; ix <= 1; ix++)
                {
                    h += ((color_t*)source.data)[(x + ix) + (y + iy) * source.width].r * kh[iy + 1][ix + 1];
                    v += ((color_t*)source.data)[(x + ix) + (y + iy) * source.width].r * kv[iy + 1][ix + 1];
                }
            }
